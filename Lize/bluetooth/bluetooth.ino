...
unsigned long lastNoteTimestamp = 0;
unsigned long dt = 0;
  ...
  while(BLUETOOTH.available()>0){
    ...
    if(i==numShiftReg){
      ...
      if (lastNoteTimestamp > 0) {
        dt = millis() - lastNoteTimestamp;
      }
      lastNoteTimestamp = millis();

      USB.print(F("Received 7 bytes: "));
      for (int j=0; j<numShiftReg; j++) {
        printHex(notes[j]); USB.print(F(" "));
      }
      USB.print(millis());
      USB.print(F(" delay:")); USB.print(dt); USB.print(F("us"));
      USB.println();
